...
import com.mindfiresolutions.trackyourfittness.MainActivities.HomeActivity;
...
public class WeighGraphFragment extends Fragment {
    @Override
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
                             Bundle savedInstanceState) {

        View rootView = inflater.inflate(R.layout.weight_graph_fragment, container, false);
        ...
        yaxis.setText(getString(R.string.prompt_weight));

        for(int i = 0; i< mHealthDetailsList.size(); i++)
        {
            values.add(new PointValue(Float.parseFloat(mHealthDetailsList.get(i).get(ID)), Float.parseFloat(mHealthDetailsList.get(i).get(WEIGHT_KEY))));
        }

//        values.add(new PointValue(0, 20));
//        values.add(new PointValue(1, 1));
//        values.add(new PointValue(2, 50));
//        values.add(new PointValue(3, 30));

        LineChartView chart = (LineChartView) rootView.findViewById(R.id.chart);

        //In most cased you can call data model methods in builder-pattern-like manner.
        Line line = new Line(values).setColor(Constants.GRAPH_RED).setCubic(true);
