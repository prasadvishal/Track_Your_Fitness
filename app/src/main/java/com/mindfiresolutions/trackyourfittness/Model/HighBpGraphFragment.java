...
public class HighBpGraphFragment extends Fragment {

    @Override
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
                             Bundle savedInstanceState) {

        View rootView = inflater.inflate(R.layout.high_bp_fragment, container, false);
        ...
        yaxis.setText(getString(R.string.prompt_high_bp));

        for(int i=0;i<mHealthDetailsList.size();i++)
        {
            values.add(new PointValue(Float.parseFloat(mHealthDetailsList.get(i).get(ID)), Float.parseFloat(mHealthDetailsList.get(i).get(HIGH_BP_KEY))));
        }
