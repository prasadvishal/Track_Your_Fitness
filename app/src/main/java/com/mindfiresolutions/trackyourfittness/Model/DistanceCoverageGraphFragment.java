...
public class DistanceCoverageGraphFragment extends Fragment {

    @Override
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
                             Bundle savedInstanceState) {

        View rootView = inflater.inflate(R.layout.distance_coverage_fragment, container, false);
        ...
        yaxis.setText(getString(R.string.km));

        for(int i=0;i<mHealthDetailsList.size();i++)
        {
            values.add(new PointValue(Float.parseFloat(mHealthDetailsList.get(i).get(ID)), Float.parseFloat(mHealthDetailsList.get(i).get(DISTANCE_KEY))));
        }

        LineChartView chart = (LineChartView) rootView.findViewById(R.id.chart);

        //In most cased you can call data model methods in builder-pattern-like manner.
        Line line = new Line(values).setColor(Constants.GRAPH_ORANGE).setCubic(true);
